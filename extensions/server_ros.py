...
def point_cloud_callback(point_cloud_msg, conn):
    ...
    custom_print(f"Sent data #{count}: size = {data_size} bytes, shape = {pc_array.shape}")
...
def send_point_cloud_continuously(host='localhost', port=12345):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with conn:
            ...
            rospy.Subscriber("/rslidar_points", PointCloud2, point_cloud_callback, callback_args=conn)
